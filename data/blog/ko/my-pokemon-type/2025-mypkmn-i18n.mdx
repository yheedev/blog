---
title: '리액트 컴포넌트에 i18n 적용하기'
date: '2025-07-15'
tags: ['React', 'i18n'']
topics: ['My Pokemon Type']
draft: false
summary: 'React 컴포넌트에 i18n 설치 및 사용하는 법'
toc: true
---

# 리액트 컴포넌트에 i18n 적용하기

tag: React, Redux, Zustand
About: My Pokemon Type
생성 일시: 2025년 7월 15일 오후 8:40

이 글을 React, Zustand, Redux 등 어떤 게시판에 올릴지 고민이 되었는데,, 앞으로 zod 같은 라이브러리에 대해 작성할 필요도 있으니까 프론트엔드 라이브러리에 대해 공부한걸 기록하는 카테고리를 추가해야겠다

mypkmn 마이그레이션 세팅 중 i8next 설치를 까먹었다가 다운로드 받았다
...
[https://github.com/stillcorners/mypokemontype/blob/f17fad8a7ec467cb5f6e916144a982852a813f1b/src/stores/useLanguageStore.tsx](https://github.com/stillcorners/mypokemontype/blob/f17fad8a7ec467cb5f6e916144a982852a813f1b/src/stores/useLanguageStore.tsx)
...
[https://github.com/stillcorners/mypokemontype/blob/f17fad8a7ec467cb5f6e916144a982852a813f1b/src/i18n.ts](https://github.com/stillcorners/mypokemontype/blob/f17fad8a7ec467cb5f6e916144a982852a813f1b/src/i18n.ts)
...
2. i8next 내부적으로 리소스에 json 파일을 등록한다. 이후 useTransition 훅이 json 파일을 참고해서 처리한다.
3. 컴포넌트에서 사용할 때에는 `{t('Mode.offense')}` 같은 방식으로 useTransition을 사용한다
4. 언어 변경 시 context 전체에 적용

장점으로는 번역 누락시 fallback을 자동으로 처리한다는 것, 다양한 서드 파티를 지원하는 것, SSR과 CSR 환경을 모두 대응 가능하다는 것이다. 또한 번역 파일을 namespace로 분리할 수 있고 backend, loadPath 같은 lazy loading을 지원한다. 중복 렌더링도 useTransition 내부에서 최적화를 처리한다.
...
i18next같은 경우에는 t() 함수에 memoized + i118next 내부에서 효율적으로 바뀐 부분만 리렌더링이 된다.
...
[https://github.com/stillcorners/mypokemontype/blob/fea7638f586899bd06a6053ad9d3a6f018950d9f/src/i18n.ts](https://github.com/stillcorners/mypokemontype/blob/fea7638f586899bd06a6053ad9d3a6f018950d9f/src/i18n.ts)
...
이 세팅과 더불어서 useLanguageStore, types/langaue.ts utils/lang.ts 등을 수정하고 더이상 translation을 사용하지 않도록 수정했다.
